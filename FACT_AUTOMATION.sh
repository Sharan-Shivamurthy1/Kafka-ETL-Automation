...
NULL_QUERY=`sqlplus -s $DB_CONNECTION <<EOF
set head off feedback off linesize 10000
SELECT dbms_lob.substr(NULL_QUERY, dbms_lob.getlength(NULL_QUERY), 1) FROM TESTING_QUERIES WHERE JOB_NAME='$JOBNAME';
exit;
EOF`
echo null query is $NULL_QUERY

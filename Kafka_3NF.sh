...
Expectedstatus=1
TESTCASE='EPC Staging to 3NF validation'
echo "v_path=$v_path"
...
echo STAGING TO 3NF EXECUTION

GetJobName $SEQ_NUM '3NF'
stringarray=($var)
...
sqlplus -s $DB_CONNECTION <<EOF
insert into process_owner_test_log(source_name,stage,interface_id,interface_name,interface_subtype,test_scenario,test_case,step_name,test_result,execute_date,interface_type)
values('$SOURCENAME','STAGING TO 3NF','$INTERFACEID','$INTERFACENAME','$SUBTYPE','POSITIVE','JOB_EXECUTION','JOB_RUN_STATUS','PASSED',SYSTIMESTAMP,'KAFKA');
exit;
EOF
...
values('$SOURCENAME','STAGING TO 3NF','$INTERFACEID','$INTERFACENAME','$SUBTYPE','POSITIVE','JOB_EXECUTION','JOB_RUN_STATUS','FAILED','JOB_RUN_STATUS is ${job_run_status}',SYSTIMESTAMP,'KAFKA');
exit;
EOF
...
sqlplus -s $DB_CONNECTION <<EOF
insert into process_owner_test_log(source_name,stage,interface_id,interface_name,interface_subtype,test_scenario,test_case,step_name,test_result,comments,execute_date,interface_type)
values('$SOURCENAME','STAGING TO 3NF','$INTERFACEID','$INTERFACENAME','$SUBTYPE','POSITIVE','DATA_VALIDATION','JOB_EXECUTION','PASSED','Data Matching',SYSTIMESTAMP,'KAFKA');
exit;
EOF
...
sqlplus -s $DB_CONNECTION <<EOF
insert into process_owner_test_log(source_name,stage,interface_id,interface_name,interface_subtype,test_scenario,test_case,step_name,test_result,comments,execute_date,interface_type)
values('$SOURCENAME','STAGING TO 3NF','$INTERFACEID','$INTERFACENAME','$SUBTYPE','POSITIVE','DATA_VALIDATION','JOB_EXECUTION','FAILED','Data is not Matching',SYSTIMESTAMP,'KAFKA');
exit;
EOF
  fi  #Data Validation
...
sqlplus -s $DB_CONNECTION <<EOF
insert into process_owner_test_log(source_name,stage,interface_id,interface_name,interface_subtype,test_scenario,test_case,step_name,test_result,comments,execute_date,interface_type)
values('$SOURCENAME','STAGING TO 3NF','$INTERFACEID','$INTERFACENAME','$SUBTYPE','POSITIVE','DATA_VALIDATION','JOB_EXECUTION','FAILED','SOURCE AND TARGET COUNT IS NOT MATCHING',SYSTIMESTAMP,'KAFKA');
exit;
EOF
fi  #Count Validation
...
DUPLICATE_DATA=`sqlplus -s $TNFDB_CONNECTION <<EOF
set head off feedback off echo off newpage none pagesize 0 linesize 10000 colsep , 
$duplicate_query;
exit;
EOF`
...
sqlplus -s $DB_CONNECTION <<EOF
insert into process_owner_test_log(source_name,stage,interface_id,interface_name,interface_subtype,test_scenario,test_case,step_name,test_result,comments,execute_date,interface_type)
values('$SOURCENAME','STAGING TO 3NF','$INTERFACEID','$INTERFACENAME','$SUBTYPE','POSITIVE','DATA_VALIDATION','JOB_EXECUTION','PASSED','No Duplicates',SYSTIMESTAMP,'KAFKA');
exit;
EOF
else
...
sqlplus -s $DB_CONNECTION <<EOF
insert into process_owner_test_log(source_name,stage,interface_id,interface_name,interface_subtype,test_scenario,test_case,step_name,test_result,comments,execute_date,interface_type)
values('$SOURCENAME','STAGING TO 3NF','$INTERFACEID','$INTERFACENAME','$SUBTYPE','POSITIVE','DATA_VALIDATION','JOB_EXECUTION','FAILED','Duplicates are present',SYSTIMESTAMP,'KAFKA');
exit;
EOF
fi
...
sqlplus -s $DB_CONNECTION <<EOF
insert into process_owner_test_log(source_name,stage,interface_id,interface_name,interface_subtype,test_scenario,test_case,step_name,test_result,comments,execute_date,interface_type)
values('$SOURCENAME','STAGING TO 3NF','$INTERFACEID','$INTERFACENAME','$SUBTYPE','POSITIVE','DATA_VALIDATION','JOB_EXECUTION','PASSED','JOB SUCCESSFUL',SYSTIMESTAMP,'KAFKA');
exit;
EOF

...
source_tab=$(echo $source | sed -e "s/,/%' OR OBJECT_NAME LIKE '%/g")
echo onject name is $source 

SOURCEOBJ=%$source_tab%
echo $SOURCEOBJ
...
then
echo JOBNAME: $job
query_source=$f2  #Getting the source query
query_target=$f3  #Getting the target query
query_null=$f4  #Getting the null query
...
NULL_DATA=`sqlplus -s $DWDDDB_CONNECTION <<EOF
set head off feedback off echo off newpage none pagesize 0 linesize 10000 colsep , 
$NULL_QUERY;
exit;
EOF`
echo null data $NULL_DATA

echo "$NULL_DATA" > $log_file_path/NULL_DATA$job_run_id.csv
if [ "$NULL_DATA" == "" ]
then
#x="'"
#source=$(echo $SOURCE_TABLE | sed -e "s/'//g")
...
#target=$(echo $TARGET_TABLE | sed -e "s/'//g")
#echo target: $target
echo NULL VALUES ARE NOT PRESENT
sqlplus -s $DB_CONNECTION <<EOF
insert into DIM_FACTS_TEST_LOG(INTERFACE_ID,DIMENTION_FACTS,SOURCE_TABLE,TARGET_TABLE,TABLE_TYPE,STAGE,TEST_CASE,STEP_NAME,TEST_RESULT,EXECUTE_DATE,COMMENTS)
values('$INTERFACE_ID','$DIMENTION_FACTS','$source','$target','$TABLE_TYPE','$STAGE','TARGET ${target} NULL VALIDATION','NULL_VALIDATION','PASSED',SYSTIMESTAMP,'IN TARGET ${target} NULL VALUES IS NOT PRESENT');
exit;
EOF
else  #null Validation
echo NULL VALUES ARE PRESENT
#x="'"
#source=$(echo $SOURCE_TABLE | sed -e "s/'//g")
#echo source: $source
#target=$(echo $TARGET_TABLE | sed -e "s/'//g")
...
sqlplus -s $DB_CONNECTION <<EOF
insert into DIM_FACTS_TEST_LOG(INTERFACE_ID,DIMENTION_FACTS,SOURCE_TABLE,TARGET_TABLE,TABLE_TYPE,STAGE,TEST_CASE,STEP_NAME,TEST_RESULT,EXECUTE_DATE,COMMENTS)
values('$INTERFACE_ID','$DIMENTION_FACTS','$source','$target','$TABLE_TYPE','$STAGE','TARGET ${target} NULL VALIDATION','NULL_VALIDATION','FAILED',SYSTIMESTAMP,'IN TARGET ${target} NULL VALUES IS  PRESENT');
exit;
EOF
